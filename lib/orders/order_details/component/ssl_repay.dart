...
class SslRepay extends StatefulWidget {
  String? orderId;

  SslRepay({required this.orderId});

  @override
  State<SslRepay> createState() => _SslRepayState();
}

class _SslRepayState extends State<SslRepay> {
  ...
  Future<int> getBkashInitials({required String? orderId}) async {
    ...
    try {
      var url = Uri.parse(AppUrl.base_url +
          "/sslcommerz/begin?payment_type=single_order_payment&order_id=$orderId");
      print(url);
